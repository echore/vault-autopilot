...
var VaultAutopilotSettingTab = class extends import_obsidian.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian.Setting(containerEl).setName("Enable HTTP server").setDesc("Allows Chrome extension and other tools to drop files via POST /clip").addToggle((t2) => t2.setValue(this.plugin.settings.httpServer.enabled).onChange(async (v2) => {
      this.plugin.settings.httpServer.enabled = v2;
      await this.plugin.saveSettings();
      this.plugin.restartServer();
    }));
    new import_obsidian.Setting(containerEl).setName("Port").setDesc("Default: 27183. Restart Obsidian after changing.").addText((t2) => t2.setValue(String(this.plugin.settings.httpServer.port)).onChange(async (v2) => {
    ...
    new import_obsidian.Setting(containerEl).setName("Add provider").setDesc("Configure at least one provider before creating rules.").addDropdown(
      (d2) => d2.addOption("cli-claude", "CLI: claude (Claude Code subscription)").addOption("cli-gemini", "CLI: gemini (Google account, free)").addOption("cli-codex", "CLI: codex (ChatGPT Plus subscription)").addOption("openai-compat", "API: OpenAI-compatible (OpenRouter / OpenAI / Grok / DeepSeek / Ollama)").addOption("anthropic", "API: Anthropic (Claude API)").addOption("gemini-api", "API: Google Gemini API")
    ).addButton((b2) => b2.setButtonText("Add").onClick(async () => {
      ...
      if (type === "openai-compat") {
        ...
        const cliType = type.replace("cli-", "");
    ...
    for (const [i2, prov] of this.plugin.settings.providers.entries()) {
      this.renderProvider(containerEl, prov, i2);
    }
    new import_obsidian.Setting(containerEl).setName("Watch Rules").setHeading();
    new import_obsidian.Setting(containerEl).setName("Add rule").addButton((b2) => b2.setButtonText("Add rule").onClick(async () => {
      var _a3;
      const id = crypto.randomBytes(4).toString("hex");
      this.plugin.settings.rules.push({ id, enabled: true, watchFolder: "", sopPath: "", outputFolder: "", providerId: ((_a3 = this.plugin.settings.providers[0]) == null ? void 0 : _a3.id) || "" });
      await this.plugin.saveSettings();
      this.display();
    }));
  ...
  renderProvider(el, prov, i2) {
    ...
    new import_obsidian.Setting(el).setName(label).setHeading();
    if (prov.type === "cli") {
      new import_obsidian.Setting(el).setName("Binary path").setDesc(`Leave empty to auto-detect. Example: /usr/local/bin/${prov.cliType}`).addText((t2) => t2.setValue(prov.bin).onChange(async (v2) => {
        this.plugin.settings.providers[i2].bin = v2.trim();
        await this.plugin.saveSettings();
      }));
    }
    if (prov.type === "openai-compat") {
      new import_obsidian.Setting(el).setName("Label").addText((t2) => t2.setValue(prov.label).onChange(async (v2) => {
        this.plugin.settings.providers[i2].label = v2;
        await this.plugin.saveSettings();
      }));
      new import_obsidian.Setting(el).setName("Base URL").setDesc("e.g. https://openrouter.ai/api/v1").addText((t2) => t2.setValue(prov.baseUrl).onChange(async (v2) => {
        this.plugin.settings.providers[i2].baseUrl = v2.trim();
        await this.plugin.saveSettings();
      }));
      new import_obsidian.Setting(el).setName("API Key").addText((t2) => t2.setValue(prov.apiKey).onChange(async (v2) => {
        this.plugin.settings.providers[i2].apiKey = v2.trim();
        await this.plugin.saveSettings();
      }));
      new import_obsidian.Setting(el).setName("Model").setDesc("Must support vision for image analysis. e.g. gpt-4o, openai/gpt-4o").addText((t2) => t2.setValue(prov.model).onChange(async (v2) => {
        this.plugin.settings.providers[i2].model = v2.trim();
        await this.plugin.saveSettings();
      }));
    }
    if (prov.type === "anthropic" || prov.type === "gemini-api") {
      ...
      new import_obsidian.Setting(el).setName("Model").addText((t2) => t2.setValue(prov.model).onChange(async (v2) => {
        this.plugin.settings.providers[i2].model = v2.trim();
        await this.plugin.saveSettings();
      }));
    }
    new import_obsidian.Setting(el).addButton((b2) => b2.setButtonText("Remove provider").onClick(async () => {
      this.plugin.settings.providers.splice(i2, 1);
      await this.plugin.saveSettings();
      this.display();
    }));
  }
  renderRule(el, rule, i2) {
    new import_obsidian.Setting(el).setName(`Rule ${i2 + 1}`).setHeading();
    new import_obsidian.Setting(el).setName("Watch folder").setDesc("Vault-relative. e.g. Inbox/screenshots").addText((t2) => t2.setValue(rule.watchFolder).onChange(async (v2) => {
      this.plugin.settings.rules[i2].watchFolder = v2.trim();
      await this.plugin.saveSettings();
    }));
    new import_obsidian.Setting(el).setName("SOP / prompt path").setDesc("Absolute path to the markdown file with instructions.").addText((t2) => t2.setValue(rule.sopPath).onChange(async (v2) => {
      this.plugin.settings.rules[i2].sopPath = v2.trim();
      await this.plugin.saveSettings();
    }));
    new import_obsidian.Setting(el).setName("Output folder").setDesc("Vault-relative. e.g. Notes/Aesthetic").addText((t2) => t2.setValue(rule.outputFolder).onChange(async (v2) => {
      this.plugin.settings.rules[i2].outputFolder = v2.trim();
      await this.plugin.saveSettings();
    }));
    new import_obsidian.Setting(el).setName("Provider").addDropdown((d2) => {
      this.plugin.settings.providers.forEach((p2) => d2.addOption(p2.id, p2.type === "cli" ? `CLI: ${p2.cliType}` : p2.label || p2.type));
      d2.setValue(rule.providerId).onChange(async (v2) => {
        this.plugin.settings.rules[i2].providerId = v2;
        await this.plugin.saveSettings();
      });
    });
    new import_obsidian.Setting(el).setName("Enabled").addToggle((t2) => t2.setValue(rule.enabled).onChange(async (v2) => {
      this.plugin.settings.rules[i2].enabled = v2;
      await this.plugin.saveSettings();
    }));
    new import_obsidian.Setting(el).addButton((b2) => b2.setButtonText("Remove rule").onClick(async () => {
      this.plugin.settings.rules.splice(i2, 1);
      await this.plugin.saveSettings();
      this.display();
    }));
  }
...
var EventStream = class {
  ...
  _emit(event, ...args) {
    ...
    if (event === "abort") {
      const error = args[0];
      if (!__classPrivateFieldGet8(this, _EventStream_catchingPromiseCreated, "f") && !(listeners == null ? void 0 : listeners.length)) {
...
function hasAutoParseableInput(params) {
  ...
  return (_b = (_a3 = params.tools) == null ? void 0 : _a3.some((t2) => isAutoParsableTool(t2) || t2.type === "function" && t2.function.strict === true)) != null ? _b : false;
}
...
_AbstractChatCompletionRunner_instances = /* @__PURE__ */ new WeakSet(), _AbstractChatCompletionRunner_getFinalContent = function _AbstractChatCompletionRunner_getFinalContent2() {
  var _a3;
  return (_a3 = __classPrivateFieldGet10(this, _AbstractChatCompletionRunner_instances, "m", _AbstractChatCompletionRunner_getFinalMessage).call(this).content) != null ? _a3 : null;
...
var ChatCompletionStream = class _ChatCompletionStream extends AbstractChatCompletionRunner {
  ...
  [(_ChatCompletionStream_params = /* @__PURE__ */ new WeakMap(), _ChatCompletionStream_choiceEventStates = /* @__PURE__ */ new WeakMap(), _ChatCompletionStream_currentChatCompletionSnapshot = /* @__PURE__ */ new WeakMap(), _ChatCompletionStream_instances = /* @__PURE__ */ new WeakSet(), _ChatCompletionStream_beginRequest = function _ChatCompletionStream_beginRequest2() {
    ...
    var _a3, _b, _c, _d, _e2, _f, _g, _h, _i, _j, _k, _l, _m, _n, _o, _p, _q, _r2, _s, _t2;
    if (this.ended)
      return;
      ...
      for (const toolCallDelta of (_p = choice.delta.tool_calls) != null ? _p : []) {
        ...
        if ((toolCallSnapshot == null ? void 0 : toolCallSnapshot.type) === "function") {
          this._emit("tool_calls.function.arguments.delta", {
            name: (_r2 = toolCallSnapshot.function) == null ? void 0 : _r2.name,
            index: toolCallDelta.index,
            arguments: toolCallSnapshot.function.arguments,
            parsed_arguments: toolCallSnapshot.function.parsed_arguments,
            arguments_delta: (_t2 = (_s = toolCallDelta.function) == null ? void 0 : _s.arguments) != null ? _t2 : ""
          });
        } else {
    ...
    if (toolCallSnapshot.type === "function") {
      const inputTool = (_c = (_b = __classPrivateFieldGet11(this, _ChatCompletionStream_params, "f")) == null ? void 0 : _b.tools) == null ? void 0 : _c.find((tool) => tool.type === "function" && tool.function.name === toolCallSnapshot.function.name);
    ...
    var _a3;
    const responseFormat = (_a3 = __classPrivateFieldGet11(this, _ChatCompletionStream_params, "f")) == null ? void 0 : _a3.response_format;
      ...
      if (tool_calls) {
        ...
        for (const { index: index2, id, type, function: fn, ...rest3 } of tool_calls) {
          ...
          if (type)
            tool_call.type = type;
          if (fn)
            (_f = tool_call.function) != null ? _f : tool_call.function = { name: (_e2 = fn.name) != null ? _e2 : "", arguments: "" };
...
function createOpenAICompatProvider(config) {
  const client = new openai_default({
    ...
    // required in Electron environment
  });
  return {
    id: config.id,
...
async function fileFromPath5(path4, ...args) {
  const { fileFromPath: _fileFromPath } = await Promise.resolve().then(() => (init_fileFromPath(), fileFromPath_exports));
...
function getRuntime2() {
  ...
  return {
    ...
    Blob: Blob2,
    File: File2,
    ReadableStream: import_web2.ReadableStream,
      ...
      try {
        for await (const sse of _iterSSEMessages2(response, controller)) {
          ...
          if (sse.event === "error") {
            ...
            const errMessage = errJSON ? void 0 : errText;
            throw APIError2.generate(void 0, errJSON, errMessage, createResponseHeaders2(response.headers));
          }
        }
        done = true;
      } catch (e2) {
...
async function toFile2(value, name, options) {
  var _a3, _b, _c;
  value = await value;
  options != null ? options : options = isFileLike3(value) ? { lastModified: value.lastModified, type: value.type } : {};
...
var APIClient2 = class {
  ...
  buildURL(path4, query) {
    const url = isAbsoluteURL2(path4) ? new URL(path4) : new URL(this.baseURL + (this.baseURL.endsWith("/") && path4.startsWith("/") ? path4.slice(1) : path4));
    const defaultQuery = this.defaultQuery();
...
var AbstractPage2 = class {
  constructor(client, response, body, options) {
    ...
    __classPrivateFieldSet11(this, _AbstractPage_client2, client, "f");
    this.options = options;
    this.response = response;
    this.body = body;
  }
  ...
  async getNextPage() {
    ...
    return await __classPrivateFieldGet13(this, _AbstractPage_client2, "f").requestAPIList(this.constructor, nextOptions);
  }
  async *iterPages() {
    let page = this;
    yield page;
...
var PromptCachingBetaMessageStream = class _PromptCachingBetaMessageStream {
  constructor() {
    ...
    _PromptCachingBetaMessageStream_handleError.set(this, (error) => {
      __classPrivateFieldSet12(this, _PromptCachingBetaMessageStream_errored, true, "f");
      if (error instanceof Error && error.name === "AbortError") {
        error = new APIUserAbortError2();
      }
      if (error instanceof APIUserAbortError2) {
        __classPrivateFieldSet12(this, _PromptCachingBetaMessageStream_aborted, true, "f");
        return this._emit("abort", error);
      }
    ...
    __classPrivateFieldGet14(this, _PromptCachingBetaMessageStream_endPromise, "f").catch(() => {
    });
  }
  /**
   * Intended for use on the frontend, consuming a stream produced with
  ...
  _run(executor) {
    executor().then(() => {
      this._emitFinal();
      this._emit("end");
    }, __classPrivateFieldGet14(this, _PromptCachingBetaMessageStream_handleError, "f"));
  }
  ...
  async _createPromptCachingBetaMessage(messages, params, options) {
    ...
    __classPrivateFieldGet14(this, _PromptCachingBetaMessageStream_instances, "m", _PromptCachingBetaMessageStream_beginRequest).call(this);
    const stream = await messages.create({ ...params, stream: true }, { ...options, signal: this.controller.signal });
    this._connected();
    for await (const event of stream) {
      __classPrivateFieldGet14(this, _PromptCachingBetaMessageStream_instances, "m", _PromptCachingBetaMessageStream_addStreamEvent).call(this, event);
    }
    if ((_a3 = stream.controller.signal) == null ? void 0 : _a3.aborted) {
      throw new APIUserAbortError2();
    }
    __classPrivateFieldGet14(this, _PromptCachingBetaMessageStream_instances, "m", _PromptCachingBetaMessageStream_endRequest).call(this);
  }
  _connected() {
    if (this.ended)
      return;
    __classPrivateFieldGet14(this, _PromptCachingBetaMessageStream_resolveConnectedPromise, "f").call(this);
    this._emit("connect");
  }
  get ended() {
    return __classPrivateFieldGet14(this, _PromptCachingBetaMessageStream_ended, "f");
  }
  get errored() {
    return __classPrivateFieldGet14(this, _PromptCachingBetaMessageStream_errored, "f");
  }
  get aborted() {
    return __classPrivateFieldGet14(this, _PromptCachingBetaMessageStream_aborted, "f");
  }
  abort() {
    this.controller.abort();
  }
   ...
   */
  on(event, listener) {
    const listeners = __classPrivateFieldGet14(this, _PromptCachingBetaMessageStream_listeners, "f")[event] || (__classPrivateFieldGet14(this, _PromptCachingBetaMessageStream_listeners, "f")[event] = []);
    listeners.push({ listener });
    return this;
  }
  /**
   ...
   */
  off(event, listener) {
    const listeners = __classPrivateFieldGet14(this, _PromptCachingBetaMessageStream_listeners, "f")[event];
    if (!listeners)
      return this;
    const index = listeners.findIndex((l2) => l2.listener === listener);
    if (index >= 0)
  ...
  async done() {
    __classPrivateFieldSet12(this, _PromptCachingBetaMessageStream_catchingPromiseCreated, true, "f");
    await __classPrivateFieldGet14(this, _PromptCachingBetaMessageStream_endPromise, "f");
  }
  get currentMessage() {
    return __classPrivateFieldGet14(this, _PromptCachingBetaMessageStream_currentMessageSnapshot, "f");
  }
  /**
  ...
  async finalMessage() {
    await this.done();
    return __classPrivateFieldGet14(this, _PromptCachingBetaMessageStream_instances, "m", _PromptCachingBetaMessageStream_getFinalMessage).call(this);
  }
  /**
  ...
  async finalText() {
    await this.done();
    return __classPrivateFieldGet14(this, _PromptCachingBetaMessageStream_instances, "m", _PromptCachingBetaMessageStream_getFinalText).call(this);
  }
  _emit(event, ...args) {
    if (__classPrivateFieldGet14(this, _PromptCachingBetaMessageStream_ended, "f"))
      return;
    if (event === "end") {
      __classPrivateFieldSet12(this, _PromptCachingBetaMessageStream_ended, true, "f");
      __classPrivateFieldGet14(this, _PromptCachingBetaMessageStream_resolveEndPromise, "f").call(this);
    }
    const listeners = __classPrivateFieldGet14(this, _PromptCachingBetaMessageStream_listeners, "f")[event];
    if (listeners) {
      __classPrivateFieldGet14(this, _PromptCachingBetaMessageStream_listeners, "f")[event] = listeners.filter((l2) => !l2.once);
      listeners.forEach(({ listener }) => listener(...args));
    }
    if (event === "abort") {
      const error = args[0];
      if (!__classPrivateFieldGet14(this, _PromptCachingBetaMessageStream_catchingPromiseCreated, "f") && !(listeners == null ? void 0 : listeners.length)) {
        Promise.reject(error);
      }
      __classPrivateFieldGet14(this, _PromptCachingBetaMessageStream_rejectConnectedPromise, "f").call(this, error);
      __classPrivateFieldGet14(this, _PromptCachingBetaMessageStream_rejectEndPromise, "f").call(this, error);
      this._emit("end");
      return;
    }
    if (event === "error") {
  ...
  _emitFinal() {
    ...
    if (finalPromptCachingBetaMessage) {
      this._emit("finalPromptCachingBetaMessage", __classPrivateFieldGet14(this, _PromptCachingBetaMessageStream_instances, "m", _PromptCachingBetaMessageStream_getFinalMessage).call(this));
    }
  }
  async _fromReadableStream(readableStream, options) {
    var _a3;
  ...
  [(_PromptCachingBetaMessageStream_currentMessageSnapshot = /* @__PURE__ */ new WeakMap(), _PromptCachingBetaMessageStream_connectedPromise = /* @__PURE__ */ new WeakMap(), _PromptCachingBetaMessageStream_resolveConnectedPromise = /* @__PURE__ */ new WeakMap(), _PromptCachingBetaMessageStream_rejectConnectedPromise = /* @__PURE__ */ new WeakMap(), _PromptCachingBetaMessageStream_endPromise = /* @__PURE__ */ new WeakMap(), _PromptCachingBetaMessageStream_resolveEndPromise = /* @__PURE__ */ new WeakMap(), _PromptCachingBetaMessageStream_rejectEndPromise = /* @__PURE__ */ new WeakMap(), _PromptCachingBetaMessageStream_listeners = /* @__PURE__ */ new WeakMap(), _PromptCachingBetaMessageStream_ended = /* @__PURE__ */ new WeakMap(), _PromptCachingBetaMessageStream_errored = /* @__PURE__ */ new WeakMap(), _PromptCachingBetaMessageStream_aborted = /* @__PURE__ */ new WeakMap(), _PromptCachingBetaMessageStream_catchingPromiseCreated = /* @__PURE__ */ new WeakMap(), _PromptCachingBetaMessageStream_handleError = /* @__PURE__ */ new WeakMap(), _PromptCachingBetaMessageStream_instances = /* @__PURE__ */ new WeakSet(), _PromptCachingBetaMessageStream_getFinalMessage = function _PromptCachingBetaMessageStream_getFinalMessage2() {
    ...
    if (this.ended)
      return;
    __classPrivateFieldSet12(this, _PromptCachingBetaMessageStream_currentMessageSnapshot, void 0, "f");
  }, _PromptCachingBetaMessageStream_addStreamEvent = function _PromptCachingBetaMessageStream_addStreamEvent2(event) {
    if (this.ended)
      return;
    const messageSnapshot = __classPrivateFieldGet14(this, _PromptCachingBetaMessageStream_instances, "m", _PromptCachingBetaMessageStream_accumulateMessage).call(this, event);
    ...
    switch (event.type) {
      ...
      case "message_start": {
        __classPrivateFieldSet12(this, _PromptCachingBetaMessageStream_currentMessageSnapshot, messageSnapshot, "f");
        break;
      }
    ...
    const snapshot = __classPrivateFieldGet14(this, _PromptCachingBetaMessageStream_currentMessageSnapshot, "f");
    if (!snapshot) {
    ...
    let snapshot = __classPrivateFieldGet14(this, _PromptCachingBetaMessageStream_currentMessageSnapshot, "f");
...
(function(PromptCaching2) {
  PromptCaching2.Messages = Messages3;
...
var MessageStream = class _MessageStream {
  constructor() {
    ...
    _MessageStream_handleError.set(this, (error) => {
      __classPrivateFieldSet13(this, _MessageStream_errored, true, "f");
      if (error instanceof Error && error.name === "AbortError") {
        error = new APIUserAbortError2();
      }
      if (error instanceof APIUserAbortError2) {
        __classPrivateFieldSet13(this, _MessageStream_aborted, true, "f");
        return this._emit("abort", error);
      }
      if (error instanceof AnthropicError) {
        return this._emit("error", error);
    ...
    __classPrivateFieldGet15(this, _MessageStream_endPromise, "f").catch(() => {
    });
  }
  /**
   * Intended for use on the frontend, consuming a stream produced with
  ...
  _run(executor) {
    executor().then(() => {
      this._emitFinal();
      this._emit("end");
    }, __classPrivateFieldGet15(this, _MessageStream_handleError, "f"));
  }
  ...
  async _createMessage(messages, params, options) {
    ...
    __classPrivateFieldGet15(this, _MessageStream_instances, "m", _MessageStream_beginRequest).call(this);
    const stream = await messages.create({ ...params, stream: true }, { ...options, signal: this.controller.signal });
    this._connected();
    for await (const event of stream) {
      __classPrivateFieldGet15(this, _MessageStream_instances, "m", _MessageStream_addStreamEvent).call(this, event);
    }
    if ((_a3 = stream.controller.signal) == null ? void 0 : _a3.aborted) {
      throw new APIUserAbortError2();
    }
    __classPrivateFieldGet15(this, _MessageStream_instances, "m", _MessageStream_endRequest).call(this);
  }
  _connected() {
    if (this.ended)
      return;
    __classPrivateFieldGet15(this, _MessageStream_resolveConnectedPromise, "f").call(this);
    this._emit("connect");
  }
  get ended() {
    return __classPrivateFieldGet15(this, _MessageStream_ended, "f");
  }
  get errored() {
    return __classPrivateFieldGet15(this, _MessageStream_errored, "f");
  }
  get aborted() {
    return __classPrivateFieldGet15(this, _MessageStream_aborted, "f");
  }
  abort() {
    this.controller.abort();
  }
   ...
   */
  on(event, listener) {
    const listeners = __classPrivateFieldGet15(this, _MessageStream_listeners, "f")[event] || (__classPrivateFieldGet15(this, _MessageStream_listeners, "f")[event] = []);
    listeners.push({ listener });
    return this;
  }
  /**
   ...
   */
  off(event, listener) {
    const listeners = __classPrivateFieldGet15(this, _MessageStream_listeners, "f")[event];
    if (!listeners)
      return this;
    const index = listeners.findIndex((l2) => l2.listener === listener);
    if (index >= 0)
  ...
  async done() {
    __classPrivateFieldSet13(this, _MessageStream_catchingPromiseCreated, true, "f");
    await __classPrivateFieldGet15(this, _MessageStream_endPromise, "f");
  }
  get currentMessage() {
    return __classPrivateFieldGet15(this, _MessageStream_currentMessageSnapshot, "f");
  }
  /**
  ...
  async finalMessage() {
    await this.done();
    return __classPrivateFieldGet15(this, _MessageStream_instances, "m", _MessageStream_getFinalMessage).call(this);
  }
  /**
  ...
  async finalText() {
    await this.done();
    return __classPrivateFieldGet15(this, _MessageStream_instances, "m", _MessageStream_getFinalText).call(this);
  }
  _emit(event, ...args) {
    if (__classPrivateFieldGet15(this, _MessageStream_ended, "f"))
      return;
    if (event === "end") {
      __classPrivateFieldSet13(this, _MessageStream_ended, true, "f");
      __classPrivateFieldGet15(this, _MessageStream_resolveEndPromise, "f").call(this);
    }
    const listeners = __classPrivateFieldGet15(this, _MessageStream_listeners, "f")[event];
    if (listeners) {
      __classPrivateFieldGet15(this, _MessageStream_listeners, "f")[event] = listeners.filter((l2) => !l2.once);
      listeners.forEach(({ listener }) => listener(...args));
    }
    if (event === "abort") {
      const error = args[0];
      if (!__classPrivateFieldGet15(this, _MessageStream_catchingPromiseCreated, "f") && !(listeners == null ? void 0 : listeners.length)) {
        Promise.reject(error);
      }
      __classPrivateFieldGet15(this, _MessageStream_rejectConnectedPromise, "f").call(this, error);
      __classPrivateFieldGet15(this, _MessageStream_rejectEndPromise, "f").call(this, error);
      this._emit("end");
      return;
    }
    if (event === "error") {
  ...
  _emitFinal() {
    ...
    if (finalMessage) {
      this._emit("finalMessage", __classPrivateFieldGet15(this, _MessageStream_instances, "m", _MessageStream_getFinalMessage).call(this));
    }
  }
  async _fromReadableStream(readableStream, options) {
    var _a3;
  ...
  [(_MessageStream_currentMessageSnapshot = /* @__PURE__ */ new WeakMap(), _MessageStream_connectedPromise = /* @__PURE__ */ new WeakMap(), _MessageStream_resolveConnectedPromise = /* @__PURE__ */ new WeakMap(), _MessageStream_rejectConnectedPromise = /* @__PURE__ */ new WeakMap(), _MessageStream_endPromise = /* @__PURE__ */ new WeakMap(), _MessageStream_resolveEndPromise = /* @__PURE__ */ new WeakMap(), _MessageStream_rejectEndPromise = /* @__PURE__ */ new WeakMap(), _MessageStream_listeners = /* @__PURE__ */ new WeakMap(), _MessageStream_ended = /* @__PURE__ */ new WeakMap(), _MessageStream_errored = /* @__PURE__ */ new WeakMap(), _MessageStream_aborted = /* @__PURE__ */ new WeakMap(), _MessageStream_catchingPromiseCreated = /* @__PURE__ */ new WeakMap(), _MessageStream_handleError = /* @__PURE__ */ new WeakMap(), _MessageStream_instances = /* @__PURE__ */ new WeakSet(), _MessageStream_getFinalMessage = function _MessageStream_getFinalMessage2() {
    ...
    if (this.ended)
      return;
    __classPrivateFieldSet13(this, _MessageStream_currentMessageSnapshot, void 0, "f");
  }, _MessageStream_addStreamEvent = function _MessageStream_addStreamEvent2(event) {
    if (this.ended)
      return;
    const messageSnapshot = __classPrivateFieldGet15(this, _MessageStream_instances, "m", _MessageStream_accumulateMessage).call(this, event);
    this._emit("streamEvent", event, messageSnapshot);
    switch (event.type) {
      case "content_block_delta": {
        const content = messageSnapshot.content.at(-1);
      ...
      case "message_start": {
        __classPrivateFieldSet13(this, _MessageStream_currentMessageSnapshot, messageSnapshot, "f");
        break;
      }
      case "content_block_start":
      case "message_delta":
    ...
    const snapshot = __classPrivateFieldGet15(this, _MessageStream_currentMessageSnapshot, "f");
    if (!snapshot) {
      throw new AnthropicError(`request ended without sending any chunks`);
    }
    __classPrivateFieldSet13(this, _MessageStream_currentMessageSnapshot, void 0, "f");
    ...
    let snapshot = __classPrivateFieldGet15(this, _MessageStream_currentMessageSnapshot, "f");
    if (event.type === "message_start") {
      if (snapshot) {
        throw new AnthropicError(`Unexpected event order, got ${event.type} before receiving "message_stop"`);
      }
...
var Anthropic = class extends APIClient2 {
  ...
  constructor({ baseURL = readEnv2("ANTHROPIC_BASE_URL"), apiKey = ((_a3) => (_a3 = readEnv2("ANTHROPIC_API_KEY")) != null ? _a3 : null)(), authToken = ((_b) => (_b = readEnv2("ANTHROPIC_AUTH_TOKEN")) != null ? _b : null)(), ...opts } = {}) {
    ...
    if (isRunningInBrowser2()) {
      throw new AnthropicError("It looks like you're running in a browser-like environment, which is disabled to protect your secret API credentials from attackers. If you have a strong business need for client-side use of this API, please open a GitHub issue with your use-case and security mitigations.");
    }
    super({
      baseURL: options.baseURL,
      timeout: (_a4 = options.timeout) != null ? _a4 : 6e5,
...
Anthropic.fileFromPath = fileFromPath4;
var { HUMAN_PROMPT, AI_PROMPT } = Anthropic;
var { AnthropicError: AnthropicError2, APIError: APIError3, APIConnectionError: APIConnectionError3, APIConnectionTimeoutError: APIConnectionTimeoutError3, APIUserAbortError: APIUserAbortError3, NotFoundError: NotFoundError3, ConflictError: ConflictError3, RateLimitError: RateLimitError3, BadRequestError: BadRequestError3, AuthenticationError: AuthenticationError3, InternalServerError: InternalServerError3, PermissionDeniedError: PermissionDeniedError3, UnprocessableEntityError: UnprocessableEntityError3 } = error_exports2;
(function(Anthropic2) {
  Anthropic2.Completions = Completions4;
  Anthropic2.Messages = Messages4;
...
var GenerativeModel = class {
  ...
  startChat(startChatParams) {
    var _a3;
    return new ChatSession(this.apiKey, this.model, Object.assign({ generationConfig: this.generationConfig, safetySettings: this.safetySettings, tools: this.tools, toolConfig: this.toolConfig, systemInstruction: this.systemInstruction, cachedContent: (_a3 = this.cachedContent) === null || _a3 === void 0 ? void 0 : _a3.name }, startChatParams), this.requestOptions);
  }
  /**
...
function detectBinaryPath(cliType, configured) {
...
async function processFile(absoluteFilePath, rule, provider, meta) {
  if (!fs3.existsSync(rule.sopPath)) {
    throw new Error(`SOP file not found: ${rule.sopPath}`);
  }
  const ext = path2.extname(absoluteFilePath).toLowerCase();
  const fileType = IMAGE_EXTS.has(ext) ? "image" : "text";
  const fileContent = readFileAsBuffer(absoluteFilePath);
  const sopContent = fs3.readFileSync(rule.sopPath, "utf8").toString();
  const request = { filePath: absoluteFilePath, fileType, fileContent, sopContent, meta };
...
function runStartupChecks(settings) {
  ...
  for (const rule of settings.rules) {
    ...
    else if (!fs4.existsSync(rule.sopPath)) problems.push(`Rule "${rule.id}": SOP file not found at "${rule.sopPath}".`);
...
var VaultAutopilotPlugin = class extends import_obsidian3.Plugin {
  ...
  onunload() {
    var _a3;
    (_a3 = this.server) == null ? void 0 : _a3.close();
    this.server = null;
  }
  async loadSettings() {
  ...
  findMatchingRule(filePath) {
    return this.settings.rules.find(
      (r2) => r2.enabled && filePath.startsWith(r2.watchFolder + "/")
    ) || null;
  }
  async handleNewFile(file, rule) {
